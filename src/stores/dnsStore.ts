...
interface DnsState {
  ...
  fetchRecords: (accountId: string, domainId: string, keyword?: string, recordType?: string) => Promise<void>;
...
export const useDnsStore = create<DnsState>((set, get) => ({
  ...
  fetchRecords: async (accountId, domainId, keyword, recordType) => {
    ...
    try {
      const response = await invoke<ApiResponse<PaginatedResponse<DnsRecord>>>(
        "list_dns_records",
        {
          accountId,
          domainId,
